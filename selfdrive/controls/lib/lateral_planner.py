...
class LateralPlanner:
  ...
  def update(self, sm):
    ...
    if not self.get_dynamic_lane_profile(sm['longitudinalPlan'], v_ego_car):
  ...
  def get_dynamic_lane_profile(self, longitudinal_plan, v_ego):
    if not self.dynamic_lane_profile_enabled:
      return True
    elif self.dynamic_lane_profile == 1:
      return True
    elif self.dynamic_lane_profile == 0:
      return True if v_ego < 5 * CV.MPH_TO_MS else False
    elif self.dynamic_lane_profile == 2:
      if v_ego < 5 * CV.MPH_TO_MS:
        return True
      # laneless while lane change in progress
      elif self.DH.lane_change_state in (LaneChangeState.laneChangeStarting, LaneChangeState.laneChangeFinishing):
        return True
